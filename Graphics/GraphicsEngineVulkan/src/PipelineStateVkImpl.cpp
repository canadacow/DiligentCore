...
void PipelineStateVkImpl::InitPipelineLayout(const PipelineStateCreateInfo& CreateInfo, TShaderStages& ShaderStages) noexcept(false)
{
    ...
    const auto VerifyBindings = !RemapResources && ((InternalFlags & PSO_CREATE_INTERNAL_FLAG_NO_SHADER_REFLECTION) == 0) && (CreateInfo.Flags & PSO_CREATE_FLAG_DONT_VERIFY_SHADER_RESOURCES) == 0;
